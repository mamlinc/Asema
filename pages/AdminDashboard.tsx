      ...
      <aside className="lg:w-72 bg-zinc-950 text-white flex flex-col sticky top-0 h-screen z-50 overflow-hidden shadow-2xl">
        ...
        <nav className="p-6 flex-grow overflow-y-auto space-y-2 custom-scrollbar">
          {[
            ...
            {id:'products',l:'المنتجات',i:'🪵'}, 
            {id:'articles',l:'المدونة',i:'✍️'}, 
            {id:'gallery',l:'المعرض',i:'🖼️'}, 
            {id:'reviews',l:'التقييمات',i:'💬'}, 
            {id:'inquiries',l:'الرسائل',i:'📬'}, 
            ...
            >
              <span className="text-xl">{t.i}</span><span className="text-sm">{t.l}</span>
            </button>
          ))}
        </nav>
      ...
      <main className="flex-grow p-6 lg:p-14 overflow-y-auto max-h-screen custom-scrollbar">
        <header className="flex flex-col md:flex-row justify-between items-center mb-16 gap-6">
           ...
           <div className="flex flex-wrap gap-4">
              {activeTab === 'products' && <button onClick={() => { setEditingId(null); setPForm({nAr:'',nEn:'',dAr:'',dEn:'',cat:'Premium',imgs:[]}); setShowProductModal(true); }} className="bg-zinc-900 text-white px-8 py-3.5 rounded-2xl text-xs font-black shadow-xl hover:bg-orange-600 transition">+ إضافة منتج</button>}
              {activeTab === 'articles' && <button onClick={() => { setEditingId(null); setAForm({tAr:'',tEn:'',cAr:'',cEn:'',img:''}); setShowArticleModal(true); }} className="bg-zinc-900 text-white px-8 py-3.5 rounded-2xl text-xs font-black shadow-xl hover:bg-orange-600 transition">+ مقال جديد</button>}
              {activeTab === 'gallery' && <button onClick={() => { setEditingId(null); setGForm({url:'',tAr:'',tEn:'',cat:'factory'}); setShowGalleryModal(true); }} className="bg-zinc-900 text-white px-8 py-3.5 rounded-2xl text-xs font-black shadow-xl hover:bg-orange-600 transition">+ رفع ميديا</button>}
              {activeTab === 'reviews' && <button onClick={() => { setEditingId(null); setRForm({author:'',rating:5,cAr:'',cEn:'',avatar:''}); setShowReviewModal(true); }} className="bg-zinc-900 text-white px-8 py-3.5 rounded-2xl text-xs font-black shadow-xl hover:bg-orange-600 transition">+ إضافة تقييم</button>}
           </div>
        </header>
        ...
        {activeTab === 'stats' && (
          <div className="space-y-12 animate-fade-up">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
               {[
                 {l:'المنتجات', v:products.length, i:'🪵', c:'text-orange-600', b:'bg-orange-50'},
                 {l:'المقالات', v:articles.length, i:'✍️', c:'text-blue-600', b:'bg-blue-50'},
                 {l:'الرسائل', v:inquiries.length, i:'📬', c:'text-green-600', b:'bg-green-50'},
                 {l:'الميديا', v:gallery.length, i:'🖼️', c:'text-purple-600', b:'bg-purple-50'}
               ].map((s,i) => (
        ...
        <div className="grid gap-4 animate-fade-up">
           ...
           {activeTab === 'products' && products.map(p => (
             <div key={p.id} className="bg-white p-6 rounded-[2.5rem] border border-zinc-100 flex items-center justify-between hover:shadow-xl transition-all group">
                ...
                <div className="flex gap-3">
                   <button onClick={() => { 
                      setEditingId(p.id); 
                      setPForm({nAr:p.name.ar,nEn:p.name.en,dAr:p.description.ar,dEn:p.description.en,cat:p.category,imgs:p.images}); 
                      setShowProductModal(true); 
                   }} className="p-4 bg-zinc-50 rounded-2xl hover:bg-zinc-100 transition text-zinc-500">✏️</button>
                   ...
                   }} className="p-4 bg-red-50 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition">🗑️</button>
                </div>
             </div>
           ))}
           ...
           {activeTab === 'articles' && articles.map(a => (
             <div key={a.id} className="bg-white p-6 rounded-[2.5rem] border border-zinc-100 flex items-center justify-between group">
                ...
                <div className="flex gap-3">
                   ...
                   }} className="p-4 bg-zinc-50 rounded-2xl">✏️</button>
                   <button onClick={async () => {
                      if(confirm('حذف المقال؟')) {
                         await apiService.deleteArticle(a.id);
                         await sync();
                      }
                   }} className="p-4 bg-red-50 text-red-500 rounded-2xl">🗑️</button>
                </div>
             </div>
           ))}
                ...
                {gallery.map(g => (
                  <div key={g.id} className="bg-white p-4 rounded-[2.5rem] border border-zinc-100 group relative overflow-hidden">
                     ...
                     <button 
                        ...
                        className="absolute top-6 right-6 bg-red-600 text-white w-10 h-10 rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-xl"
                     >🗑️</button>
                  </div>
                ))}
             </div>
           ...
           {/* Settings View */}
           {activeTab === 'settings' && (
             <div className="bg-white p-14 rounded-[4rem] border border-zinc-100 shadow-sm max-w-4xl space-y-10 animate-in slide-in-from-bottom">
                <div className="grid md:grid-cols-2 gap-10">
                   ...
                   <div className="md:col-span-2 space-y-4">
                      ...
                      <input value={cfgForm.heroTitle.ar} onChange={e => setCfgForm({...cfgForm, heroTitle: {...cfgForm.heroTitle, ar: e.target.value}})} className="w-full bg-zinc-50 p-5 rounded-2xl border-0 focus:ring-2 focus:ring-orange-600 outline-none text-2xl font-black" />
                   </div>
                   <div className="md:col-span-2 space-y-4">
                ...
                <button 
                   ...
                   disabled={isSaving}
                   className="w-full bg-orange-600 text-white py-7 rounded-3xl font-black text-xl shadow-2xl shadow-orange-600/20 hover:bg-orange-500 transition-all active:scale-95"
                >
        ...
        {showProductModal && (
          <div className="fixed inset-0 z-[1000] bg-zinc-950/95 backdrop-blur-xl flex items-center justify-center p-6 overflow-y-auto">
             <div className="bg-white w-full max-w-5xl rounded-[4rem] p-12 relative animate-fade-up">
                <button onClick={() => setShowProductModal(false)} className="absolute top-10 right-10 text-5xl font-light hover:text-orange-600 transition-colors">&times;</button>
                ...
                <div className="grid md:grid-cols-2 gap-12">
                   <div className="space-y-6">
                      ...
                      <div className="space-y-2">
                         ...
                         <textarea value={pForm.dAr} onChange={e => setPForm({...pForm, dAr: e.target.value})} className="w-full bg-zinc-50 p-5 rounded-2xl h-48 resize-none" />
                      </div>
                   </div>
                   <div className="space-y-6">
                      <div className="space-y-2">
        ...
        {showArticleModal && (
          <div className="fixed inset-0 z-[1000] bg-zinc-950/95 backdrop-blur-xl flex items-center justify-center p-6 overflow-y-auto">
             <div className="bg-white w-full max-w-4xl rounded-[4rem] p-12 relative animate-fade-up">
                ...
                <div className="space-y-6">
                   <input value={aForm.tAr} onChange={e => setAForm({...aForm, tAr: e.target.value})} placeholder="عنوان المقال" className="w-full bg-zinc-50 p-6 rounded-2xl text-xl font-black" />
                   ...
                   <textarea value={aForm.cAr} onChange={e => setAForm({...aForm, cAr: e.target.value})} placeholder="محتوى المقال..." className="w-full bg-zinc-50 p-6 rounded-2xl h-80 resize-none font-medium text-lg leading-relaxed" />
        ...
        {showGalleryModal && (
          <div className="fixed inset-0 z-[1000] bg-zinc-950/95 backdrop-blur-xl flex items-center justify-center p-6">
             <div className="bg-white w-full max-w-md rounded-[4rem] p-12 relative animate-fade-up">
                <button onClick={() => setShowGalleryModal(false)} className="absolute top-8 right-8 text-3xl">&times;</button>
        ...
        {showReviewModal && (
          <div className="fixed inset-0 z-[1000] bg-zinc-950/95 backdrop-blur-xl flex items-center justify-center p-6">
             <div className="bg-white w-full max-w-md rounded-[4rem] p-12 relative animate-fade-up">
                <button onClick={() => setShowReviewModal(false)} className="absolute top-8 right-8 text-3xl">&times;</button>
